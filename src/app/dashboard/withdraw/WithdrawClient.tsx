    ...
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-500 font-medium">Loading withdrawal data...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="w-full min-w-0 space-y-6 sm:space-y-8 animate-fade-in pb-24 md:pb-8">
      ...
      <div className="grid lg:grid-cols-5 gap-6 w-full min-w-0">
        ...
        <div className="lg:col-span-2 space-y-6 w-full min-w-0">
          <div className="bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-3xl p-6 sm:p-8 shadow-lg shadow-indigo-200/50 relative overflow-hidden text-white h-full flex flex-col justify-between min-h-[320px]">
            ...
            <div className="relative z-10">
              <div className="space-y-3 bg-black/10 p-4 rounded-2xl border border-white/10 backdrop-blur-sm">
                <div className="flex items-start gap-3">
                  <AlertCircle className="w-5 h-5 text-indigo-200 flex-shrink-0 mt-0.5" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-indigo-100 leading-relaxed">
                      Withdrawals are subject to a <strong className="text-white">{withdrawalFeePct}% platform fee</strong>.
                      Minimum withdrawal amount is <strong className="text-white">${minWithdrawalUsd}</strong>.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <div className="flex flex-wrap justify-between items-center gap-2">
                  ...
                  <button
                    onClick={handleMax}
                    className="text-[10px] font-bold uppercase tracking-wider bg-sky-50 text-sky-600 px-2 py-1 rounded-md hover:bg-sky-100 transition-colors whitespace-nowrap"
                  >
                    Max Amount
                  </button>
                </div>
                <div className="relative group">
                  ...
                  <input
                    id="amount"
                    type="number"
                    className="block w-full pl-11 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-lg font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all"
              ...
              <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 overflow-hidden">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-200/50 flex items-center justify-center flex-shrink-0">
                  <Wallet className="w-4 h-4 sm:w-5 sm:h-5 text-slate-500" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] sm:text-xs font-semibold text-slate-500 uppercase tracking-wider mb-0.5">Receiving Address</p>
                  <p className="text-xs sm:text-sm font-mono text-slate-800 truncate">{address || 'Not connected'}</p>
                </div>
              </div>
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${amount > 0 ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="bg-sky-50/50 border border-sky-100 rounded-2xl p-4 sm:p-5 space-y-3 sm:space-y-4">
                  <div className="flex flex-wrap justify-between items-center text-sm gap-2">
                    ...
                    <span className="text-slate-900 font-bold">${amount.toFixed(2)}</span>
                  </div>
                  <div className="flex flex-wrap justify-between items-center text-sm gap-2">
                    <span className="text-slate-500 font-medium flex-shrink-0">Platform Fee ({withdrawalFeePct}%)</span>
                    <span className="text-rose-500 font-bold">-${fee.toFixed(2)}</span>
                  </div>
                  <div className="h-px bg-sky-200/50 w-full my-1 sm:my-2"></div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                    <span className="text-slate-700 font-bold">You Receive (Approx)</span>
                    <div className="text-left sm:text-right w-full min-w-0">
                      <div className="text-lg sm:text-xl font-black text-sky-600 truncate">{finalAmountTrx} TRX <span className="text-sm font-bold text-sky-500/70 whitespace-nowrap">(BEP-20)</span></div>
                      <div className="text-xs font-bold text-slate-400 mt-0.5">≈ ${finalAmountUsd.toFixed(2)} USD</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                <div className="relative flex items-center justify-center gap-2 z-10 w-full min-w-0">
                  {isProcessing ? (
                    <>
                      <RefreshCw className="h-5 w-5 animate-spin text-sky-400 flex-shrink-0" />
